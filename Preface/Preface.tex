...
\pharo は \st プログラミング言語・環境をフル機能で実装した現代的かつオープンソースな処理系です。 \pharo は、古典的な \st-80 システムの再実装である\squeak\cite{Inga97a}から派生しました。\squeak が主に実験的な教育向けソフトウェアを開発するためのプラットフォームとして作られたのに対して、\pharo はプロフェッショナルなソフトウェア開発のためのリーンなオープンソース・プラットフォーム、また、動的な言語・環境の研究開発のための健全で安定したプラットフォームを目指しています。\pharo はSeasideウェブ開発フレームワークのリファレンス実装用にも使われています。
...
\pharo は \squeak からパッケージをいくつも取り去っている一方で、\squeak ではオプションであった機能をたくさん取り入れています。たとえば、\pharo には True Type フォントが入っています。\pharo は本物のブロッククロージャもサポートしています(訳注:\squeak ではBlockContextという再帰できないクロージャ実装でした)。ユーザインターフェースもシンプルで洗練したものになっています。

\pharo は極めて高い移植性を持っています。仮想マシンでさえもすべて \st で書かれていて、デバッグ・解析・変更をしやすくなっています。\pharo はマルチメディア・アプリケーションから、教育向けプラットフォーム、商用ウェブ開発環境まで、広範囲のイノベーティブなプロジェクトの媒体となります。

\pharo の裏にはある重要な視点があります。「\pharo は過去の単なる複製であってはならない。Smalltalkを\emph{再発明}すべきだ」というものです。ビッグバンアプローチは滅多に成功しません。\pharo では、進化的でインクリメンタルな変化をさせる方法を取ります。重要な新機能やライブラリを実験的に進められるようにしたいのです。進化とは誤りを受け入れることです。\pharo では一発で完璧に解決することは目指しません。一発解決を目指したくなる気持ちをこらえてでも、です。小さな変化をいくつも積み上げていくやり方が、\pharo では上手くいくでしょう。\pharo の成功はコミュニティからの貢献にかかっているのです。
...
その名の通り、\pharo と\squeak の違いを反映させて修正をしました。
...
\important{{\bf 気にしない}
\st プログラミングの初心者が陥りやすい罠は、どうやってそれが動くのかを詳細まですべて理解しなければならないと考え、困ってしまうことです。そんなことをしていたら、\ct{Transcript show: 'Hello World'} をマスターするまでにかなり時間がかかってしまいます。オブジェクト指向の優れた点は、「どうやって動くのだろう?」という問いに対して「気にしない」と答えられることなのです。}
%=================================================================
