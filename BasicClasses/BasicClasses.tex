...
Behavior>>>new
    "添え字付きの変数を持たないレシーバ(クラス)の初期化された新しいインスタンスを返す。
    クラスが添え字付きの場合は失敗する。"
    ^ self basicNew initialize
\end{method} %@ 自信なし。Collection で、indexable を「添え字付き」と機械的に訳していた。
...
\clsindmain{String} クラスも \scatind{Collections-Strings} カテゴリで定義されています。\ct{String} は、\ct{Character} だけを保持する添え字付きの \ct{Collection} です。
