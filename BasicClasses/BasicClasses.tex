...
本節では、\ct{Object} のインスタンスボタン側のメソッドカテゴリ(プロトコル)をブラウズして行きます。それにともない、\ct{Object} が提供するいくつかの主だった振る舞いが理解できるようになって行くでしょう。
