...
\clasind{Interval}は数列の始点から終点をイテレートするコレクションです．
...
コレクションは関数型の畳み込み演算\pmph{fold}を\ct{inject:into:}メソッドによりサポートします．
...
\stにおいては，すべてはオブジェクトです．そして，あらゆることがメッセージを送ることにより発生します．
しかし，ある地点で底に当たります．
...
プリミティブを呼び出すメソッドはさらに\stコードを含んでいることがあります，そのようなコードはプリミティブが失敗した場合のみ評価されます．
...
\pharoでは，角括弧シンタックスもプラグマと呼ばれるメソッド・アノテーションに使われます．
...
\pharoには擬似変数と呼ばれる6つ(だけ)の予約識別子があります．
...
\item		5種類のリテラル・オブジェクトがあります．
		数(\ct{5}, \ct{2.5}, \ct{1.9e15}, \ct{2r111})，文字(\ct{$a})，文字列(\ct{'hello'})，シンボル(\ct{#hello})および配列(\ct{#('hello' #hi)})
