...
\item[カスケード。] セミコロンは、一つのレシーバにメッセージの\emphind{カスケード}(列)を送るのに使うことができます。\ct{Transcript show: 'hello'; cr} は、最初に \ct{Transcript} にキーワードメッセージ \ct{show: 'hello'} を送り、次に同じレシーバ(\ct{Transcript})に単項メッセージ \ct{cr} を送ります。
	\seeindex{;}{カスケード}
...
\ct{super} も、現在実行しているメソッドのレシーバを指します。しかし、\ct{super} にメッセージを送る場合、メソッド探索は、そのメソッドを定義しているクラスのスーパークラスから始まります。
...
一連のメッセージを\emph{同じ}レシーバに送る場合、\emphind{カスケード}としてより簡潔に表現することができます。
...
条件式は、真偽値にメッセージ \mthind{Boolean}{ifTrue:}、\mthind{Boolean}{ifFalse:} あるいは \mthind{Boolean}{ifTrue:ifFalse:} を送ることにより表します。真偽値に関する詳細については、\charef{basic}を参照してください。
...
数に、メッセージ \mthind{Number}{to:do:} を送って、その数をループカウンタの初期値とすることもできます。
...
\item	ブロッククロージャーは、角括弧で囲まれた式です。
		...
		ブロック中の式は、ブロックに \ct{value...} メッセージと正しい個数の引数を送るまで、
		評価されません。\\
		\ct{[:x | x + 2] value: 4 --> 6}。
