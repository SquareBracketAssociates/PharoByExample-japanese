...
例えば if-then-else 制御構造の代わりに、\st では \clsind{Boolean} オブジェクトに \ct{ifTrue:} のようなメッセージを送信します。
...
式は以下の構成要素でできています。
(i) 6 個の予約語、すなわち \pvind{self}、\pvind{super}、\pvind{nil}、\pvind{true}、\pvind{false}、\pvind{thisContext}の\emph{擬似変数}
...
\item[ローカル変数] \ct{startPoint} は変数名、つまり識別子です。
		...
		これにより変数の有効範囲がプライベートということが読み手に伝わります。
...
\item[シンボル] は文字の並びでできている点で文字列に似ています。
	しかし、文字列と異なり、\subind{リテラル}{シンボル}はグローバルに一意であることが保証されます。
...
\item[動的配列] 中括弧 \ct|{ }|は実行時に(\subind{配列}{動的})配列を定義します。
...
\item[ブロック] 角括弧 \ct{[ ]} はブロッククロージャーあるいはレキシカルクロージャーとも呼ばれる\ind{ブロック}を定義します。ブロックは関数を表す第一級のオブジェクトです。
...
\item[単項メッセージ] は単一の単語(例えば \ct{factorial})から成り、レシーバ(例えば \ct{3})に送信されます。
	\index{メッセージ!単項}
...
\item[キーワードメッセージ] はraisedTo:modulo:のように、複数のキーワードから成るメッセージです。キーワードはコロンで終わり、それぞれ引数を取ります。 \ct{2 raisedTo: 6 modulo: 10} の式では、\ct{raisedTo:modulo:} がメッセージセレクタで、 コロンの後の\ct{6} と \ct{10} が引数です。メッセージは2に送られています。
	\index{メッセージ!キーワード}
...
これらは事前に定義され代入することができないため、\subind{変数}{擬似}変数と呼ばれます。
...
\begin{enumerate}
  ...
  \item \emph{二項}メッセージは引き数を 1つだけ取ります。
  	\ct{1 + 2} は、オブジェクト \ct{1} にメッセージ \ct{+} を送信します。引数は \ct{2} です。
  \item \emph{キーワード}メッセージは 1つ以上の引数を取ります。
...
二項メッセージのメッセージセレクタは、1文字以上で記述します。以下の文字を組み合わせることができます。
\index{メッセージ!二項}
\begin{code}{}
+ - / \ * ~ < > = @ % | & ! ? ,
\end{code}
...
キーワードメッセージのメッセージセレクタは、一連のキーワードを連結したものです。キーワードはそれぞれ小文字で始まりコロンで終わります。
\index{メッセージ!キーワード}
...
式を評価した結果を示すのに\lct{\emph{式}}\ct{-->}\lct{\emph{結果}}という記法を使うことにしているのを思い出してください。
...
式は \pharo のあらゆる場所(例えばデバッガ、ワークスペース、ブラウザなど)で評価することができますが、通常、メソッドはブラウザかデバッガで定義します。
(メソッドは外部からファイルインすることもできますが、これは \pharo でプログラムするには普通の方法ではありません)。

プログラムは、あるクラスに属するメソッド単位で開発していきます。
...
\begin{enumerate}
  \item 名前 (\ie \ct{lineCount}) と引数(この例では一つも無い)からなるメッセージパターン
...
ブロックは本質的には無名の関数です。ブロックを評価するには、メッセージ \mthind{BlockClosure}{value} を送信します。
ブロックは、明示的なリターン(\ct{^} で示す)が無ければ、最後の式の値を返します。
...
ブロックは引数を持つことができます。引数は先頭にコロンをつけて宣言します。
縦棒でブロック本体と引数の宣言とを区切ります。
引数が 1個のブロックを評価するには、ブロックに、引数1つのメッセージ \mthind{BlockClosure}{value:} を送信しなければなりません。
引数が 2個のブロックを評価するには、ブロックに \mthind{BlockClosure}{value:value:}を送信しなければなりません。同様にして 4 個までの引数を渡すことができます。
...
引数の数が 4 個より多いブロックには \mthind{BlockClosure}{valueWithArguments:} を使って、引数を配列にして渡さなければなりません。
...
\ct{select:} と \ct{reject:} は、真偽値の条件を満たす(あるいは満たさない)ブロックを満たす要素からなる新しいコレクション(部分集合)を作ります。
...
+ aNumber 
  ...
  必須。メソッド探索無し。Object のドキュメント whatIsAPrimitive を参照のこと。"
...
\item	ブロッククロージャーは角括弧で囲まれた式です。
		...
		ブロック内の式は、ブロックに、\ct{value...} メッセージを、適切な個数の引数で送るとはじめて
		評価されます。\\
		\ct{[:x | x + 2] value: 4 --> 6}。

\item	制御構造用の文法はありません。ある条件でブロックを評価させるメッセージ送信で実現します。\\
