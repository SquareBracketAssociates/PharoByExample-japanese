...
ほとんどのオープンソースのプロジェクトでは、コードをリポジトリで保守することができる\ind{CVS}\footnote{\url{http://www.nongnu.org/cvs}}や\ind{Subversion}\footnote{\url{http://subversion.tigris.org}}といったツールを使っています。\pharo プログラマも同様に、\ind{Monticello}パッケージによるコードの管理のほうが、より便利だと思うことでしょう。Monticellonoパッケージではファイル名の末尾に \ct{.mcz}が付きます。このファイルは\ind{パッケージ}内のコードを単にZIPで圧縮したものです。
