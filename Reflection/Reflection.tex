...
より技術的には、次のような意味です: 実行時システムの\emphind{メタオブジェクト}を通常のオブジェクトのように\emph{具現化}することができ、その結果、それらにクエリを送ったり、それらをインスペクトすることができるようになります。
%@The metaobjects in \st are classes, metaclasses, method dictionaries, compiled methods, the run-time stack, and so on.
\st のメタオブジェクトは、クラス、メタクラス、メソッド辞書、コンパイル済みメソッド、実行時スタックなどなどです。
...
このことは\emph{代祷(インターセッション?)}とも呼ばれ、おもに動的なプログラミング言語がこれをサポートしますが、静的な言語ではサポートの範囲は非常に限られます。
...
イントロスペクションとは、オブジェクト、クラス、メソッド、そして実行時スタックといったデータ構造を\emph{調べる}ことができる能力のことです。これらのデータ構造は、その言語を定義するものです。
...
インターセッションとは、こうしたデータ構造が変更できる、言い換えれば、言語のセマンティクスやプログラムのふるまいをプログラム自身の中から\emph{変更}できる能力のことです。
...
\emph{構造的リフレクション}と言ったとき、それは、実行時システムの構造を調べ・変更することを指し、\emphind{振る舞い的リフレクション}と言ったとき、それは、実行時システムの構造を解釈するしかたを変更することを指します。
...
ここでは多くの実際的な例: \st がどのようにイントロスペクションとメタプログラミングをサポートしているかを例証する実際的な例を探求します。
...
インスペクターを使えば、オブジェクト(の中身?)を見ることや、インスタンス変数の値を変更することができますし、そのオブジェクトにメッセージを送ることさえできます。
...
The \ct!contents!インスタンス変数はワークスペースがそのテキスト領域に表示しているものを、何であれ表わしているので、それを選択すれば、右側には、空文字列が表示されることになります。
...
このウィンドウを更新する合図を送るには、インスペクターの下の部分で、\ct!self contentsChanged!を評価します。
...
実は、インスペクターはアクセサがなくてもインスタンス変数にアクセスすることができ、それは\st のリフレクティブな(?)能力を用いているからです。
%@In \st, classes define instance variables either by name or by numeric indices.
\st では、インスタンス変数を、名前か数値インデックスで定義します。
...
インスペクターは\ct!Object!クラスで定義されているインスタンス変数にアクセスするためのメソッド用います。これら: \lct{instVarAt: \emph{index}}そして\lct{instVarNamed: \emph{aString}}は、\lct{\emph{index}}による位置や\lct{\emph{aString}}によってそれぞれ識別されるインスタンス変数の値を得るのに用いることができ; インスタンス変数に新しい値を代入するにには、\ct!instVarAt:put:!や\ct!instVarNamed:put:!を用いる(ことができる?)のです。
