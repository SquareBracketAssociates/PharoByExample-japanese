...
\important{\emph{警告:} これらのメソッドは、開発ツールを構築するのに便利ではありますが、通常のアプリケーションを開発するのにこれらのメソッドを使うのは、悪い考えです: これらのリフレクティブなメソッドは、オブジェクトのカプセル化の境界を破り、その結果、コードはわかりづらく、保守しづらいものになりかねません。}
...
\mthindex{オブジェクト}{instanceVariableValues}
