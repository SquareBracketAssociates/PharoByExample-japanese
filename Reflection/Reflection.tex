...
\st は、リフレクティブな(自分自身を知る)プログラミング言語です。
...
このことは\emph{インターセッション}とも呼ばれ、おもに動的なプログラミング言語がこれをサポートしますが、静的な言語ではサポートの範囲は非常に限られます。

%@A program that manipulates other programs (or even itself) is a \emphind{metaprogram}.
他のプログラム(あるいは自分自身までも)を操作するプログラムは、\emphind{メタプログラム}です。
...
インターセッションとは、こうしたデータ構造が変更できる、言い換えれば、言語のセマンティクスやプログラムのふるまいを、プログラム自身の中から\emph{変更}できる能力のことです。
...
\emph{構造的リフレクション}と言ったとき、それは、実行時システムの構造を調べ・変更することについて言うのであり、\emphind{振る舞い的リフレクション}と言ったとき、それは、実行時システムの構造を解釈するしかたを変更するについて言うのです。
...
ここでは多くの実際的な例: \st がどのようにイントロスペクションとメタプログラミングをサポートしているかを例証する、実際的な例について探求します。
...
The \ct!contents!インスタンス変数はワークスペースがそのテキスト領域に表示しているものを、それが何であれ、表わしているので、\ct!contents!を選択すれば、右側には、空文字列が表示されることになります。
...
\dothis{それでは空文字列のところに\ct!'hello'!と入力してから、\emph{accept}してみましょう。}
...
\ct!contents!の値は変りますが、ワークスペースウィンドウは、そのことに気付きません。だからワークスペースは何も再描画しません。
%@To trigger the window refresh, evaluate \ct!self contentsChanged! in the lower part of the inspector.
このウィンドウに自分自身を更新する合図を送るには、インスペクターの下の部分で、\ct!self contentsChanged!を評価します。
...
実は、インスペクターは、アクセサがなくてもインスタンス変数にアクセスすることができます。それは\st のリフレクティブな能力を用いているからです。
%@In \st, classes define instance variables either by name or by numeric indices.
\st では、クラスは、インスタンス変数を、名前か数値インデックスで定義します。
...
インスペクターは、\ct!Object!クラスで定義されている、インスタンス変数にアクセスするためのメソッド用います。これら: \lct{instVarAt: \emph{index}}そして\lct{instVarNamed: \emph{aString}}は、\lct{\emph{index}}による位置や、\lct{\emph{aString}}によって識別されるインスタンス変数の値を得るのに用いることがでます。また、インスタンス変数に新しい値を代入するにには、\ct!instVarAt:put:!や\ct!instVarNamed:put:!を用いるのです。%@?用いることができるのです。
...
たとえば、以下を評価することにより、最初のワークスペースで\ct!w!にバインドされたオブジェクトの内部状態を変更することができます: %@?自信なし
\begin{code}{}
...
\important{\emph{警告:} いくらこれらのメソッドが、開発ツールを構築するのに便利でるとは言っても、通常のアプリケーションを開発するのにこれらのメソッドを使うのは、悪い考えです: これらのリフレクティブなメソッドは、オブジェクトのカプセル化の境界を破り、その結果、コードはわかりづらく、保守しづらいものになりかねません。}
...
プリミティブ呼び出しのあとのコードは、プリミティブが失敗したときにのみ、実行されます。今のこの例では、存在しない変数にアクセスしようとしたとき、プリミティブに続くコードが試行されます。
...
いくらプリミティブメソッドのコードは変更できると言っても、そのような仕事は、\pharo システムの安定性を脅かしかねない危険な仕事であることに注意してください。%@?business: 「仕事」以外のいい訳はないか?
...
同様の考えで、特定の属性を持ったインスタンスを集めることが可能です。%@?In the same spirit: 同様の考えで?
...
たとえば、SketchMorph!クラスのインスタンスで、その\ct!owner!インスタンス変数がワールドモーフに設定されている\ct!すべてのインスタンス(\ie 現在表示されているすべてのイメージ)を得るには、次の式を試してみましょう:
\begin{code}{}
...
このメソッドは、以下のように、\ct{Object}クラスで実装されています:
\needlines{9}
\begin{code}{}
...
このメソッドは、そのクラスが定義する一連のインスタンス変数のインデックスについて、繰り返し処理します。このときのインデックスは、スーパークラスで用いられていた最後のインデックスの直後の値から始まります。
%@(The method \ct!instSize! returns the number of all named instance variables that a class defines.)
(この\ct!instSize!メソッドは、そのクラスが定義しているすべての名前付きのインスタンス変数の数を返します。) %@? 。)or)。
...
ここでは、クラスがそのメソッドを辞書に格納していることが確認できます。セレクターが、辞書のインデックスになっています。
