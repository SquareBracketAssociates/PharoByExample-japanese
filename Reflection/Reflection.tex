...
\lct{respondsTo: \emph{aSymbol}}は、レシーバーのクラスに、セレクタが\lct{\emph{aSymbol}}であるようなメソッドが定義されているときに、trueを返します。%@?receiver has a method: レシーバーのクラスに...メソッドが定義されている?
%@For instance:
たとえば:
\needlines{3}
\begin{code}{@TEST}
...
Morph selectors size.             -->  998 "新しいメソッドの数"
%@Morph instVarNames size.     -->      6 "number of new variables"
Morph instVarNames size.     -->      6 "新しい変数の数"
%@Morph subclasses size.          -->    45 "direct subclasses"
Morph subclasses size.          -->    45 "直接のサブクラス"
%@Morph allSubclasses size.      -->  326 "total subclasses"
Morph allSubclasses size.      -->  326 "サブクラスの総計"
...
\lct{Collection}の階層のメソッドのうち、どのようなメソッドが、そのセレクタと異なるメッセージを\super に送っているか、それを見出すことに関心があったとしましょう。
...
\dothis{ブラウザーを開けて、\ct{Collection}.を見てください。
