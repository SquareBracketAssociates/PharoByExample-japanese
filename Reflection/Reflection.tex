...
ランタイムシステムの\emphind{メタオブジェクト}を通常のオブジェクトとして\emph{具現化(reify))}することができるので、メタオブジェクトに問い合わせを行ったり、インスペクトして内容を見るたりすることができます。
...
メソッド中で\ct{thisContext}が参照された時にはいつでも、そのメソッドのコンテキストが具現化され、リンクした一連の\clsind{MethodContext}オブジェクトとしてイメージから操作できるようになります。
...
\clsind{ObjectsAsMethodsExample}クラス定義を調べると次の２つのメソッドが見つかります:
