...
インスペクターは、\ct!Object!クラスで定義されている、インスタンス変数にアクセスするためのメソッド用います。これら: \lct{instVarAt: \emph{index}}そして\lct{instVarNamed: \emph{aString}}は、インスタンス変数の値を得るのに用いることができるのですが、インスタンス変数の識別には、\lct{\emph{index}}による位置や、\lct{\emph{aString}}が用いることができます。また、\ct!instVarAt:put:!や\ct!instVarNamed:put:!は、インスタンス変数に新しい値を代入するのに、を用いるのです。
