...
\ct{ObjectsAsMethodsExample}は、この\ct{run:with:in:}メソッドを実装しているので、これはメッセージをインターセプトして、自分自身メッセージをに移譲します。

%@We can now remove the fake method as follows:
ここで、この偽のメソッドは、以下のようにして、削除することができます:
\begin{code}{}
...
メソッドラッパーは、メッセージをインターセプトするための、良く知られたテクニックです\cite{Bran98a}。
...
オリジナルの実装\footnote{http://www.squeaksource.com/MethodWrappers.html}では、メソッドラッパーとして、\ct{CompiledMethod}のサブクラスのインスタンスを使います。インストールされるとメソッドラッパーは、オリジナルのメソッドを呼び出す前かあとに、特別なアクションを実行することができます。
%@When uninstalled, the original method is returned to its rightful position in the method dictionary.
アンインストールされると、オリジナルのメソッドは、メソッド辞書内の、本来あるべき位置に戻されます。
...
そして、\ct{run:with:in:}メソッドは、単に、\ct{hasRun}インスタンス変数を更新し、ラッパー(それ自身)をアンインストールし(カバレッジが検証されたから)、オリジナルのメソッドにメッセージを再送します。%@?resends the message to the original method?
\begin{code}{}
...
\emphind{プラグマ}は、プログラムについてのデータを明記する注釈ですが、プログラムの実行には関わらりません。プラグマはそれらがアノテート(注釈)するメソッドの操作に直接の影響を及ぼしません。
%@Pragmas have a number of uses, among them:
プラグマにはいくつもの利用法があります、それらはたとえば以下のようなものです:
\begin{itemize}
...
\item コンパイラーへの情報: \indmain{プラグマ}は、コンパイラがメソッド呼び出しをプリミティブ関数にするのに用いられます。この関数は、バーチャルマシンか外部プラグインによって定義されなければなりません。
%@\item Runtime processing: Some pragmas are available to be examined at runtime.
\item 実行時処理: いくつかのプラグマは、実行時の検査に使うことができます。%@?プログラムの実行には関わらないのではなかったか?
\end{itemize}
...
プラグマはプログラムのメソッド宣言にのみ、適用することができます。メソッドは、1個かそれ以上のプラグマを宣言することができ、プラグマはどのSmalltalkの文よりも先に宣言されなければなりません。各プラグマは基本的にはリテラル引数をともなった静的メッセージ送信です。
...
我々は、この章の最初の方でプリミティブを紹介したときに、簡単なプラグマの例を見ました。プリミティブはプラグマ宣言以上の何物でもありません。 
...
\ct{instVarAt:}内の\ct{<primitive: 73>}を考慮してください。このプラグマのセレクターは\ct{primitive:}であり、その引数はリテラル値、\ct{73}です。
...
コンパイラーはおそらく、プラグマの大口ユーザです。SUnitはまたひとつの、アノテーションを活用するツールです。SUnitはテストユニットからアプリケーションのカバレッジを見積ることができます。いくつかのメソッドをカバレッジから除外したいと思う人がいるかもしれません。これは、\ct!SplitJointTest class!の\ct!documentation!メソッドのケースです:
...
単に、メソッドを\ct!<ignoreForCoverage>!プラグマでアノテートするだけで、カバレッジの範囲をコントロールすることができます。
