...
各メソッドは名前を持っていて、\emphsubbind{メソッド}{セレクタ}と呼ばれています。名前はそのクラスの中でユニークです。
...
これは、\id{Java}や\ind{C++}と対照的です。それらの言語では、インスタンス変数(``フィールド''とか``メンバ変数''とも呼ばれていますね)は同じクラスの他のインスタンスからのアクセスを許しています。
...
全てのメソッドは\subbind{メソッド}{パブリック}です。\footnote{正確には、ほぼ全て、です。\pharoでは、\ct{pvt}で始まるセレクタを持つメソッドはプライベートで、\ct{pvt}メッセージは\selfに\emph{のみ}送ることができます。しかし、\ct{pvt}メソッドはあまり使われていません。}
