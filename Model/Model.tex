...
もっと言えば、クラスは第二級のオブジェクトではありません。クラスは正真正銘ファーストクラスオブジェクトで、メッセージを投げたりインスペクトしたりすることができます。
...
各メソッドは名前を持っていて、\emphsubind{メソッド}{セレクター}と呼ばれています。名前はそのクラスの中でユニークです。
...
\st では、``\ind{アクセサー}メソッド''を定義しない限り、同じクラスの2つのインスタンスはお互いのインスタンス変数にアクセスすることはできません。
他のオブジェクトのインスタンス変数に直接アクセスするための言語構文がないのです。
(\ind{リフレクション}と呼ばれる機構が他のオブジェクトにインスタンス変数の値へのアクセスを提供しています。
メタプログラミングはオブジェクト\ind{インスペクター}のようなツールを記述することを意図しています。インスペクターはオブジェクトの中身を見ることを目的にしたツールです。)
...
\subsubsection{例題}
\cmind{Point}{dist:} (\mthref{dist:})メソッドは、レシーバーともう1つの点との間の距離を計算します。
レシーバーのインスタンス変数\ct{x}と\ct{y}は、メソッドから直接アクセスされています。
...
例えば、\ct{Point>>dist:}メソッドは、引数\ct{aPoint}がレシーバーと同じクラスなのかどうか知る必要もありませんし、気にする必要もないからです。
...
全てのメソッドは\subind{メソッド}{パブリック}です。\footnote{正確には、ほぼ全て、です。\pharo では、\ct{pvt}で始まるセレクターを持つメソッドはプライベートで、\ct{pvt}メッセージは\self に\emph{のみ}送ることができます。しかし、\ct{pvt}メソッドはあまり使われていません。}
メソッドは、その目的に応じてプロトコルにグループ分けされています。
共通して使われているプロトコルはコンベンションによって確立されてきました。例えば、\protind{accessing}はアクセサーメソッドで、\protind{initialization}はオブジェクトの一貫した初期状態を確立するメソッドです。
...
\st 開発者の中には、アクセサーを通してのみインスタンス変数にアクセスすることを好む人もいます。
...
このことから、ブラウザー\index{ブラウザー}は、クラスとメタクラスがあたかも1つのものの2つの側面であるかのように、クラスとメタクラスの両方をブラウズできるようになっています。\figref{Buttons}に示されている、``\subind{ブラウザー}{インスタンス側}''と\subind{ブラウザー}{クラス側}''です。
...
aColor        --> Color blue
aColor red  --> 0.0         "インスタンス側のアクセサーメソッドred"
aColor blue --> 1.0        "インスタンス側のアクセサーメソッドblue"
\end{code}

\subind{ブラウザー}{インスタンス側}で提供されるテンプレートを埋めることでクラスを定義します。
...
余計なインスタンスを生成することなく実行できるので、\subind{ブラウザー}{クラス側}にユーティリティ的なメソッドを置くと便利です。
...
このことから、インスタンスを初期化するメソッドは常に\subind{ブラウザー}{インスタンス側}で定義しなければなりません --- \subind{ブラウザー}{クラス側}からはインスタンス変数にアクセスできないので、初期化できません！
クラスができることは、新しく生成されたインスタンスに、\ind{初期化}メッセージを送信することと、可能ならばアクセサーを使うことです。

同じようにインスタンスからクラスインスタンス変数にアクセスするには、クラスにアクセサーメッセージを送ることによって間接的にアクセスするしかありません。
...
ブラウザーの\button{instance}ボタンをクリックして、\clsind{WebServer}クラスを以下のように定義します。(\clsref{singleton})
...
そして、\button{class}ボタンをクリックして、\subind{ブラウザー}{クラス側}でインスタンス変数\ct{uniqueInstance}を追加します。
...
\pharo には多重継承はありませんが、互いに関係のないクラスで同じ振る舞いを共有する\emphind{トレイト}{}と呼ばれるメカニズムをサポートしています。
...
Character>>>< aCharacter 
	"レシーバーがaCharacterより小さければ、trueと答える。"
...
メッセージを受けたレシーバーは、そこで、頼まれたことを実行するための\emph{メソッド}を選択して、どう対応するのかを決めます。
...
とても単純な過程をたどります：
メッセージを受けたレシーバーのクラスはそのメッセージを処理するのに用いるメソッドを探索します。
...
\begin{method}[defaultColor]{レシーバーのクラス自体に実装されたメソッド}
EllipseMorph>>>defaultColor
	"レシーバーのデフォルトの色/塗り方を答える"
	^ Color yellow
\end{method}
...
そのことを、``\self を答える''と言います。Smalltalkでは疑似変数\self はメッセージを受けたレシーバーを指します。\ind{Java}の\ct{this}みたいなものです。
\index{変数!疑似}
\index{return}
...
\self と同様に、\super はメッセージを受けたレシーバーを表わします。
違うのは、\ind{メソッド探索}だけです。
\super 送信があると、レシーバーのクラスから探索を始めるのではなく、そのメソッドを定義しているクラスのスーパークラスから探索を始めます。

\super はスーパークラス\emph{ではない}ことに注意してください！
スーパークラスのことだと思うのは、よく見られる間違いです。
また、レシーバーのスーパークラスから探索を始めるというのも、間違いです。
どういうことか、以下に例を示します。
...
注意していただきたいのは、\self 送信ではレシーバーのクラス、この場合には\ct{anEllipse}クラスから、メソッド探索を始めます。

\important{\self 送信は、\emph{動的な}メソッド探索を、レシーバーのクラスから開始します。}
...
\super 送信の探索はレシーバーのスーパークラスから開始しているのではない、ということに注意してください。
もしレシーバーのスーパークラスから探索を開始すると、\ct{BorderedMorph}から探索を始めるということになり、無限ループになってしまいます!
...
対照的に、\self 送信は動的です。今実行しているメッセージのレシーバーを表わしています。つまり、\self に送られた\emph{全ての}メッセージは、そのレシーバーのクラスから探索を開始します。
...
これは、クラス変数へのアクセサーメソッドを実装して、その中でもしその変数がまだ初期化されていなければ初期化することで実現できます。
その場合には、常にアクセサーメソッドを使って、クラス変数には絶対に直接アクセスしてはいけないことになります。
これは、アクセサーの送信の上に初期化する必要があるかどうかチェックするコストが上乗せされます。
...
\subind{ブラウザー}{クラス側} \ct{initialize}メソッドはコードをメモリーにロードした時には自動的に実行されますが、ブラウザーで入力してコンパイルした時や編集して再コンパイルした時には自動的には実行\emph{されない}のです。
...
\begin{itemize}
  ...
  そういったアクセスが必要ならば、アクセサーを定義しなければなりません。
  ...
  \item 全てのことはメッセージ送信で起こる。
	...
	するとレシーバーがそのメッセージに対応するメソッドを選択します。

  \item メソッド探索は継承の連鎖を辿る。
  \self 送信は動的で、レシーバーのクラスからメソッド探索を始めます。一方、
