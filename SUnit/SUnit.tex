...
\ct{ExampleSetTest} に \ct{Set} クラスのテストをまとめます。テスト実行時のコンテキスト (テストを実行するための状態) を定義？コンテキストは二つのインスタンス変数 \ct{full} と \ct{empty} で表します。 \ct{full} は要素を持つセット、 \ct{empty} は空のセットを表します。
...
このテストではコンテキストのことを \emph{フィクスチャ} と呼びます。
...
SUnit は次の 4 つのクラスから構成されます (図 7.3) 。
...
テストリソースは SUnit 3.1 で導入された概念で、コストの高いセットアップに使われます。 TestResource の setUp メソッドは TestCase のものと異なり、テストスイートの実行前に一度だけ実行されます。 TestCase では各テストの実行前に setUp メソッドが実行されます。
...
TestCase は抽象クラスです。サブクラスにテストを記述し、テスト間で実行時の状態を共有します (テストスイート) 。各テストは次の順序で実行されます。

1. TestCase のサブクラスのインスタンスを生成する (テストごとに生成します) 。
2. setUp メソッドを実行する。
3. テストメソッドを実行する。
4. tearDown メソッドを実行する。
...
実行時の状態をインスタンス変数で表し、オーバーライドした setUp メソッドで初期化します。 setUp で初期化したオブジェクトの終了処理を行うには、 tearDown メソッドをオーバーライドします。
...
TestSuite はテストケース (テストとテストスイート) の集合を含みます。つまり、 TestSuite は TestCase と TestSuite のインスタンスを含みます。
TestCase と TestSuite の間に継承関係はありませんが、どちらも同じプロトコルを実装しているので同じように扱えます。例えば、どちらも run メソッドが定義されています。 TestSuite を枝、 TestCase を葉として Composite パターンを適用しています (詳しくはデザインパターンを参照してください) 。
...
TestResult は TestSuite の実行結果を扱います。成功したテストの数、失敗したテストの数、エラーの発生したテストの数を記録します。
