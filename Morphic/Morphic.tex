...
このメソッドは\ct{drawOn:}と同じ方法を使います。要は、\ct{containsPoint:}が\ct{true}と答える領域は、\ct{drawOn}によって色が付けられた領域と一致すると言えるからです。
...
解決方法は、これらの計算を新たな２つのメソッドに分けることです。どちらも\ct{private}プロトコルに格納します。
...
再度、\ct{Rectangle}クラスでこの面倒な仕事をするメソッドを見つけます。\ct{r1 areasOutside: r2}は、\ct{r2}の外側に出ている\ct{r1}の部分を含んだ、四角形の配列を返します。
以下が改良されたコードになります。
...
\ct{drawOn:}メソッドは、サイコロの背景を描くための\ct{super}-sendと、目を描くという２つの仕事を行います。
